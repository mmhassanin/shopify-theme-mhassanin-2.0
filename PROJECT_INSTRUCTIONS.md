...
- **Slideshow Height Logic**: Implemented dynamic height support in `section-slideshow.liquid` for all three modes: `fix-height` (responsive max-width scaling), `auto-height` (dynamic aspect-ratio from first slide), and `full-height` (`100vh`). Reinforced with global stretching rules (`height: 100% !important`) for internal slide sub-elements to ensure zero-gap rendering.
...
- 2026-04-22 09:35: Implemented Custom Fluid Typography for Slideshow captions. Added range settings (Min, Preferred, Max) to style_paragraph and style_button blocks. Injected CSS clamp() logic via --fluid-font-size variable in snippet-slideshow-style.liquid. Updated assets/section-slideshow.css to handle inheritance, ensuring smooth typography scaling across viewport widths without disrupting existing static font-size fallback.
