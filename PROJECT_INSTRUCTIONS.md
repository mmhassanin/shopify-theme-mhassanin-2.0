...
- `section-list-collections-v2`: Standardized layout architecture for "Boxed vs Wide" support. Purged all destructive inline styles (`width: 100% !important`, `margin: 0 !important`) and aggressive CSS overrides that were forcing breakout. The section now relies on standard Liquid wrapper class injection (`{{ section.settings.section_layout }}`) for native Electro grid compliance on all templates.
